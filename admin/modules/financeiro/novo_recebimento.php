...
// Fetch All Members (Lawyers and Interns) for dropdown
$lawyers = $pdo->query("
    (SELECT id, nome_completo, numero_registo, 'advogado' as mtype FROM advogados)
    UNION
    (SELECT id, nome_completo, numero_registo, 'estagiario' as mtype FROM advogados_estagiarios)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $member_data = explode(':', $_POST['member_combined']);
    $adv_id = $member_data[0];
    $mtype = $member_data[1];
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO finan_pagamentos (advogado_id, membro_tipo, tipo_pagamento_id, valor_pago, data_pagamento, metodo_pagamento, observacoes, comprovativo_arquivo, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'confirmado')");
        $stmt->execute([$adv_id, $mtype, $tipo_id, $valor, $data, $metodo, $obs, $comprovativo]);
        $last_id = $pdo->lastInsertId();
        
        require_once __DIR__ . '/../../includes/LogHelper.php';
        LogHelper::log($pdo, 'FINANCE', "Registou pagamento #$last_id para $mtype ID $adv_id", 'finan_pagamentos', $last_id);

        header("Location: recibo.php?id=$last_id&success=1");
        exit;
    ...
    <div class="card-body p-5">
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="row">
                <div class="col-lg-8">
                    <div class="mb-4">
                        <label class="form-label text-uppercase fw-bold text-muted small">Beneficiário (Advogado ou Estagiário)</label>
                        <select name="member_combined" class="form-select border-0 bg-light p-3 select2-advogado shadow-sm" required>
                            <option value="">Pesquisar por Nome ou Cédula...</option>
                            <?php foreach($lawyers as $l): ?>
                                <option value="<?php echo $l['id']; ?>:<?php echo $l['mtype']; ?>">
                                    [<?php echo strtoupper($l['mtype']); ?>] <?php echo $l['nome_completo']; ?> (<?php echo $l['numero_registo']; ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="tipo_pagamento_id" id="tipo_select" class="form-select border-0 bg-light p-3 shadow-sm" required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="observacoes" class="form-control bg-light border-0 shadow-sm" rows="5" placeholder="Ex: Pagamento referente ao semestre 2024.1..."></textarea>
                    </div>
                </div>
                ...
                <div class="col-lg-4">
                    <div class="card bg-light border-0 p-4 shadow-sm">
                        <div class="mb-4">
                            ...
                            <input type="number" name="valor_pago" id="valor_input" step="0.01" class="form-control border-0 fw-bold text-success fs-4 shadow-sm" placeholder="0.00" required>
                        </div>
                        
                        <div class="mb-4">
                            <label class="form-label text-uppercase fw-bold text-muted small">Data do Pagamento</label>
                            <input type="date" name="data_pagamento" class="form-control border-0 py-2 shadow-sm" value="<?php echo date('Y-m-d'); ?>" required>
                        </div>

                        <div class="mb-4">
                            <label class="form-label text-uppercase fw-bold text-muted small">Método</label>
                            <select name="metodo_pagamento" class="form-select border-0 py-2 shadow-sm">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="file" name="comprovativo" class="form-control border-0 bg-white shadow-sm" accept="image/*,application/pdf">
                        </div>

                        <hr class="my-4">

                        <button type="submit" class="btn btn-login w-100 py-3 mb-2 shadow-sm text-uppercase fw-bold">Confirmar e Gerar Recibo</button>
