...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE orgaos_sociais SET nome = ?, cargo = ?, biografia = ?, foto = ?, mandato_inicio = ?, mandato_fim = ?, ordem_exibicao = ?, ativo = ?, superior_id = ?, orgao_diretivo_id = ? WHERE id = ?");
        $stmt->execute([$nome, $cargo, $biografia, $foto, $inicio, $fim, $ordem, $ativo, $superior_id, $diretivo_id, $id]);
