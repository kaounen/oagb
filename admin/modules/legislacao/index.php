...
$label = $tipo === 'internacional' ? 'Internacional' : 'Nacional';
try { $items = $pdo->query("SELECT * FROM $table ORDER BY ordem ASC")->fetchAll(); } catch (PDOException $e) { $items = []; }
?>
<div class="row mb-5 align-items-center">
    <div class="col-md-6">
        <h2 class="page-title">Legislação <?php echo $label; ?></h2>
        <div class="text-muted small">Gerir diplomas e instrumentos jurídicos.</div>
        <div class="mt-2">
            <a href="?tipo=nacional" class="btn btn-sm <?php echo $tipo==='nacional'?'btn-dark':'btn-outline-secondary'; ?> me-1">Nacional</a>
            <a href="?tipo=internacional" class="btn btn-sm <?php echo $tipo==='internacional'?'btn-dark':'btn-outline-secondary'; ?>">Internacional</a>
        </div>
    </div>
    <div class="col-md-6 text-md-end mt-3 mt-md-0">
        <a href="add.php?tipo=<?php echo $tipo; ?>" class="btn btn-login w-auto px-4"><i class="fas fa-plus me-2"></i>Novo Diploma</a>
    </div>
</div>
<div class="card border-0 shadow-sm"><div class="card-body p-0"><div class="table-responsive">
    <table class="table table-hover align-middle mb-0">
        <thead class="bg-light"><tr>
            ...
            <th class="border-0 small text-uppercase"><?php echo $tipo==='internacional'?'Organização':'Categoria'; ?></th>
