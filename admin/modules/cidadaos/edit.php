...
if (isset($_GET['delete_gallery'])) {
    GalleryHelper::delete($pdo, 'cidadaos', $_GET['delete_gallery']);
    header("Location: edit.php?id=" . $id . "&gal_deleted=1");
    exit;
}

$stmt = $pdo->prepare("SELECT * FROM info_cidadaos WHERE id = ?"); $stmt->execute([$id]); $item = $stmt->fetch(PDO::FETCH_OBJ);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto1']) && $_FILES['foto1']['error'] === UPLOAD_ERR_OK) {
        ...
        $new_filename = uniqid('cid_') . '.' . $file_ext;
    ...
    if (empty($errors)) {
        $pdo->prepare("UPDATE info_cidadaos SET titulo=?,slug=?,icone=?,conteudo=?,ordem=?,status=?,imagem_destaque=?,resumo=?,ficheiro_anexo=?,legenda_anexo=? WHERE id=?")
            ->execute([$titulo,$slug,$icone,$conteudo,$ordem,$status,$imagem,$resumo,$ficheiro,$legenda_anexo,$id]);

        if (isset($_FILES['attachments'])) AttachmentHelper::save($pdo, 'cidadaos', $id, $_FILES['attachments'], $_POST['attachment_descriptions'] ?? []);
        if (isset($_POST['att_desc'])) foreach ($_POST['att_desc'] as $att_id => $desc) AttachmentHelper::update($pdo, $att_id, $desc);
        if (isset($_FILES['gallery_files'])) GalleryHelper::save($pdo, 'cidadaos', $id, $_FILES['gallery_files'], $_POST['new_gal_title'] ?? [], $_POST['new_gal_desc'] ?? []);
        if (isset($_POST['gal_title'])) {
            foreach ($_POST['gal_title'] as $img_id => $title) {
                $desc = $_POST['gal_desc'][$img_id] ?? ''; $order = $_POST['gal_order'][$img_id] ?? 0;
                GalleryHelper::update($pdo, 'cidadaos', $img_id, $title, $desc, $order);
            }
        }
...
?>
<div class="row mb-4"><div class="col"><h2 class="page-title">Editar Secção Cidadãos #<?php echo $id; ?></h2><a href="index.php" class="text-muted small"><i class="fas fa-arrow-left me-1"></i>Voltar</a></div></div>
...
<div class="card border-0 shadow-sm"><div class="card-body p-4"><form method="POST" enctype="multipart/form-data"><div class="row g-3">
    <div class="col-md-8">
        <div class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label fw-bold small">Slug</label><input type="text" name="slug" class="form-control" value="<?php echo htmlspecialchars($item->slug); ?>"></div>
    ...
    <div class="col-md-4">
        <div class="card bg-light border-0"><div class="card-body p-4">
            <div class="mb-4">
                ...
                <div class="input-group">
                    ...
                    <select name="icone" class="form-select" onchange="document.querySelector('#icon-preview i').className = this.value">
                        <?php 
                        $icons = ['fas fa-balance-scale'=>'Justiça','fas fa-users'=>'Cidadãos','fas fa-info-circle'=>'Informação','fas fa-briefcase'=>'Profissional','fas fa-file-contract'=>'Documentos'];
            ...
            <?php $type = 'cidadaos'; $gallery = GalleryHelper::get($pdo, 'cidadaos', $id); require __DIR__ . '/../../includes/partials/gallery_form.php'; ?>
            <?php $entity_type = 'cidadaos'; $entity_id = $id; require __DIR__ . '/../../includes/partials/attachments_form.php'; ?>
