...
// Fetch trainees for selected letter (only those with active quotas in day)
$stmt = $pdo->prepare("SELECT ae.numero_registo, ae.nome_completo, ae.regiao, ae.localidade, ae.telefone, ae.email, ae.data_inicio_estagio, ae.foto 
                       FROM advogados_estagiarios ae
                       WHERE ae.status = 'ativo' 
                         AND ae.nome_completo LIKE ? 
                         AND EXISTS (
                             SELECT 1 FROM finan_pagamentos fp 
                             WHERE fp.advogado_id = ae.id 
                               AND fp.membro_tipo = 'estagiario' 
                               AND fp.tipo_pagamento_id = 1 
                               AND fp.status = 'confirmado' 
                               AND fp.valid_until >= CURDATE()
                         )
                       ORDER BY ae.nome_completo ASC");
...
$alfabeto = range('A', 'Z');
$contagem = [];
$stmt_count = $pdo->prepare("SELECT COUNT(*) FROM advogados_estagiarios ae 
                             WHERE ae.status = 'ativo' AND ae.nome_completo LIKE ?
                               AND EXISTS (
                                   SELECT 1 FROM finan_pagamentos fp 
                                   WHERE fp.advogado_id = ae.id 
                                     AND fp.membro_tipo = 'estagiario' 
...
$page_title = "Advogados Estagiários Inscritos em Vigor";
...
<div style="overflow-x: hidden; width: 100%; position: relative;">
    ...
    $mobile_breadcrumbs = [
        ...
        ['label' => 'Estagiários', 'url' => 'pesquisa-estagiarios.php'],
    ...
    <section class="py-5" style="background: #f7f5f0;">
        <div class="container py-lg-3">
            ...
            <div class="text-center">
                <span class="section-label" style="font-size:0.7rem; letter-spacing:4px; text-transform:uppercase; font-weight:700; color:var(--primary-gold); display:block; margin-bottom:12px;">Advogados Estagiários</span>
                <h2 class="section-heading" style="font-family:'Libre Baskerville', serif; color:var(--primary-maroon); font-weight:700; font-size:1.3rem; margin-bottom:40px;">Lista de Estagiários Inscritos</h2>
                ...
                <div class="results-summary">
                    ...
                    <span style="font-weight:700; color:var(--primary-maroon); font-size:0.9rem;">
                        Letra <?php echo $letra_atual; ?>: <?php echo count($estagiarios); ?> estagiários encontrados
                    </span>
            ...
            <div class="row g-4">
                <?php if (count($estagiarios) > 0): foreach ($estagiarios as $est): ?>
                    <div class="col-lg-4 col-md-6">
                        <div class="lawyer-card shadow-sm">
                            ...
                            <div class="lawyer-footer">
                                <a href="tel:<?php echo $est->telefone; ?>" class="btn btn-lawyer-call btn-lawyer"><i class="fas fa-phone me-2"></i> Ligar</a>
                                <a href="mailto:<?php echo $est->email; ?>" class="btn btn-lawyer-email btn-lawyer"><i class="fas fa-paper-plane me-2"></i> Email</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; else: ?>
                    <div class="col-12 text-center py-5">
                        <div class="p-5 bg-white rounded-4 border shadow-sm">
                            ...
                            <p class="text-muted">Não foram encontrados estagiários ativos cujo nome comece pela letra <?php echo $letra_atual; ?>.</p>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="search-cta wow fadeInUp">
                <div>
                    <h5 class="fw-bold mb-2 text-white">Procura um estagiário específico?</h5>
                    <p class="mb-0 opacity-75">Utilize a nossa pesquisa avançada para filtrar por região ou nome.</p>
                </div>
                <a href="pesquisa-estagiarios.php" class="btn btn-gold flex-shrink-0">Pesquisa Avançada <i class="fas fa-search ms-2"></i></a>
            </div>
