...
<head>
    ...
    <style>
        ...
        .lightbox-close:hover { color: #fff; background: rgba(77,28,33,0.8); transform: scale(1.1); }
        .gallery-thumb-wrapper { overflow: hidden; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); cursor: pointer; }
        .gallery-thumb { width: 100%; height: 180px; object-fit: cover; transition: transform 0.5s ease; }
        .gallery-thumb:hover { transform: scale(1.08); }
    </style>
</head>
<body>
                ...
                <div class="col-lg-8">
                    <?php if (count($seccoes) > 0): ?>
                        <?php foreach ($seccoes as $sec): ?>
                            <?php 
                                $attachments = AttachmentHelper::get($pdo, 'conteudos', $sec->id);
                                $gallery = GalleryHelper::get($pdo, 'conteudos', $sec->id);
                                                ...
                                                <div class="row g-3">
                                                    <?php foreach($gallery as $img): ?>
                                                        <div class="col-6 col-md-4">
                                                            <div class="gallery-thumb-wrapper" onclick="openLightbox('uploads/<?php echo $img['imagem']; ?>', '<?php echo htmlspecialchars(addslashes($img['legenda'] ?? '')); ?>')">
                                                                <img src="uploads/<?php echo $img['imagem']; ?>" class="gallery-thumb w-100" alt="<?php echo htmlspecialchars($img['legenda'] ?? ''); ?>">
                                                            </div>
                                                        </div>
                                                    <?php endforeach; ?>
                                                </div>
